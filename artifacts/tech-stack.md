...
| Infrastructure as Code | AWS CDK (TypeScript) | Three stacks: `TuracoChorusStack` (DynamoDB tables) and `TuracoChorusComputeStack` (ECS cluster/ASG/task/secret), kept separate so tearing down compute never risks the tables, plus `TuracoChorusGithubOidcStack` (one-time CI/CD bootstrap: OIDC trust, ECR repo, deploy role) — see `ecs-deployment.md` |
