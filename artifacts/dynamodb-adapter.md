...
Design for the three DynamoDB-backed adapters (see `domain-interfaces-and-objects.md` for all three ports):

- `DynamoDbLogDataSource` (behind `ILogDataSource`) — generic, config-driven per installer; maps any installer's table to `AggregateStats` purely through configuration, no adapter code changes needed. Covered in its own section below.
- `DynamoDbConsentStore` (behind `IConsentStore`) — fixed schema, owned and created by Turaco Chorus itself; no installer-facing config beyond a table name. Covered in its own section below.
- `DynamoDbAskAuditLogger` (behind `IAuditLogger`) — fixed schema, also owned and created by Turaco Chorus itself; append-only. Covered in its own section below.

## `DynamoDbLogDataSource` (`ILogDataSource`)
...
- `DateAttribute` (and `Dimensions` generally) is one flat, global config value applied to every entry regardless of its `typeId` — a heterogeneous schema where the meaningful date lives in a per-type field (found via Logger's World's own schema) can't be expressed today. See `roadmap.md`'s "Later" section for the fuller spectrum this points at (per-type config vs. a fully custom `ILogDataSource` adapter).

## `DynamoDbConsentStore` (`IConsentStore`)

Own table, construct id `TuracoChorusConsent`. One row per user, written only once a decision is made; no row means never decided. Overwritten in place on every subsequent change (see `domain-interfaces-and-objects.md`'s `ConsentRecord.GrantedAt` semantics: it's set on every change, granted or revoked, so `null` means only "never decided").
...
## `DynamoDbAskAuditLogger` (`IAuditLogger`)

Own table, construct id `TuracoChorusAskAudit`. Append-only; one row per `/ask` call, never updated or deleted.
...
SK  timestamp            (S, ISO-8601, millisecond precision)
    queryText            (S)
    consentGranted       (BOOL)
    aggregatedDataSent   (M, nested `AggregateStats`; omitted when null)
```

Known, low-probability limitation on this sort key (a same-millisecond collision between two `/ask` calls from the same user) — see `roadmap.md`'s "Later / Further Development § Known limitations".
