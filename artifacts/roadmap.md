...
- [x] Buffer for whatever slipped through
    - [x] Reconcile the non-colocated `Lookup` dimension's IAM story: granted `dynamodb:GetItem` alongside `dynamodb:Query` on the same already-scoped ARNs (`compute-stack.ts`), and corrected `dynamodb-adapter.md`'s "Read access: IAM policy" section to match — the `BatchGetItem`/caching alternative stays deferred under the doc's own "Known limitations". Code-complete on `fix/non-colocated-lookup-iam`; `cdk deploy TuracoChorusComputeStack` still pending
    - [x] Wired up CI-triggered deploy: `ecs:UpdateService`/`ecs:DescribeServices` scoped onto the GitHub Actions deploy role (`github-oidc-stack.ts`), `ClusterArnOutput`/`ServiceNameOutput` added to `compute-stack.ts`, and a `deploy` job added to `ci.yml` after `build-and-push`. `cdk deploy TuracoChorusGithubOidcStack` done and verified live (role's policy confirmed via `aws iam get-role-policy`); merging this PR is the first real trigger of the new job
